...
namespace Genesis.Core
{
    ...
    public class Utils
    {
        ...
        public static System.Numerics.Matrix4x4 ConvertToMatrix4x4(mat4 mat4)
        {
        ...
        public static int GetTriangleIndex(Element3D element, Vec3 hitpoint)
        {
            foreach (var mesh in element.Meshes)
            {
                for (int i = 0; i < mesh.Indicies.Count; i += 3)
                {
                    ...
                    if (Utils.IsPointInTriangle(hitpoint.ToVector3(), v0.ToVector3(), v1.ToVector3(), v2.ToVector3()))
                    {
        ...
        public static Vector4 CalculateUVTransform(float textureWidth, float textureHeight, float clipX, float clipY, float clipWidth, float clipHeight)
        {
